...
import { syncClearCart } from "@/libs/cart-sync";
...
const Cart = () => {
  ...
  const areAllItemsSelected = useAppSelector(selectAreAllCartItemsSelected);
  const [productMap, setProductMap] = React.useState<Record<string, Product>>({});

  React.useEffect(() => {
    const slugs = Array.from(
      new Set(cartItems.map((item) => item.productSlug).filter(Boolean))
    ) as string[];
    ...
    Promise.all(
      ...
      const nextMap = entries.reduce<Record<string, Product>>((acc, entry) => {
        if (entry) {
          acc[entry[0]] = entry[1];
        }
        return acc;
      ...
      {cartItems.length > 0 ? (
                ...
                <div className="min-w-[1170px]">
                  ...
                  <div className="flex items-center py-5.5 px-7.5">
                    <div className="min-w-[400px]">
                      <Checkbox
                        ...
                        onChange={(checked) => dispatch(setAllCartItemSelections(checked))}
                        label="Sản phẩm"
                      />
                    </div>
                  ...
                  {cartItems.length > 0 &&
                    cartItems.map((item, key) => (
                      <SingleItem
                        item={item}
                        product={item.productSlug ? productMap[item.productSlug] : undefined}
                        key={key}
                      />
                    ))}
                </div>
        ...
        <>
          <div className="text-center mt-8">
            ...
            <Link
              href="/products"
              className="inline-flex font-medium text-dark bg-blue py-3 px-6 rounded-lg ease-out duration-200 hover:bg-blue-dark"
            >
              Tiếp tục mua sắm
            </Link>
          </div>
