...
import { getCategories } from "@/libs/catalog-api";
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        const liveCategories = data.filter(
          (category) => !category.slug.startsWith("seed-category-")
        );
        const categorySource = liveCategories.length > 0 ? liveCategories : data;
        const rootCategoryIds = new Set(
          categorySource
            .filter((category) => !category.parentId)
            .map((category) => category.id)
        );
        const firstLevelCategories = categorySource.filter(
          (category) => category.parentId && rootCategoryIds.has(category.parentId)
        );

        const curatedCategories = (firstLevelCategories.length > 0
          ? firstLevelCategories
          : categorySource.filter((category) => category.parentId))
          .sort((left, right) => {
        ...
        setError("Không tải được danh mục lúc này.");
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <section className="overflow-hidden pt-17.5">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0 pb-15 border-b border-gray-3">
          <SectionLoader title="Đang tải danh mục" columns={4} compact />
        </div>
      </section>
    );
  }
      ...
      <section className="overflow-hidden pt-17.5">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0 pb-15 border-b border-gray-3">
          <div className="rounded-2xl border border-dashed border-gray-3 bg-white px-6 py-10 text-center text-dark">
            {error || "Chưa có danh mục để hiển thị."}
          </div>
        </div>
      </section>
    );
